...
tokenizer = AutoTokenizer.from_pretrained("/root/autodl-tmp/Llama3-8B-Chinese-Chat")
model = AutoModelForCausalLM.from_pretrained("/root/autodl-tmp/Llama3-8B-Chinese-Chat", torch_dtype="auto", device_map="auto")
messages = []
import pandas as pd
import re
import os
...
for year in years:
  ...
  #message = input("")      
      messages = [{"role": "system", "content": "You are Llama3-8B-Chinese-Chat, which is finetuned on Llama3-8B-Instruct with Chinese-English mixed data by the ORPO alignment algorithm. You, Llama3-8B-Chinese-Chat, is developed by Shenzhi Wang (王慎执 in Chinese). You are a helpful assistant."},{"role": "user", "content": Questions[i]},]

      input_ids=tokenizer.apply_chat_template(messages,add_generation_prompt=True,return_tensors="pt").to(model.device)

      outputs = model.generate(input_ids,max_new_tokens=8196,do_sample=True,temperature=0.6,top_p=0.9,)
      response = outputs[0][input_ids.shape[-1]:]
      answers_from_model =tokenizer.decode(response, skip_special_tokens=True)
      messages = [] 
    ...
    # print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\n"+"Answer_from_text_davinci_003:",answer1.strip().replace("\n",""),"\nAnswer_from_:GPT3.5Turbo",answer2.strip().replace("\n",""),"\nAnswer_from_GPT4:",answer3.strip().replace("\n",""))
      print("No Question",i+1,"\n","Right_Answer:",Answers[i],"\nAnswer_from_Llama3-8B-Chinese-Chat:",answer1.strip().replace("\n",""))
  ...
  save_df_to_excel(df, "Answers_from_knowledge_Llama3-8B-Chinese-Chat_EEE_"+year+".xlsx", "sheet1")
